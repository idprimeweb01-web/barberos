...
async def shot(ctx, url, nome, w=1280, h=800, wait=2000):
    pg = await ctx.new_page(viewport={"width": w, "height": h})
    await pg.goto(url)
...
async def main():
    async with async_playwright() as p:
        ...
        ctx_sa = await browser.new_context()
        ...
        await shot(ctx_pub, 'http://127.0.0.1:5000/b/caio/',         'pub_index',   480, 860)
        await shot(ctx_pub, 'http://127.0.0.1:5000/b/caio/agendar',  'pub_agendar', 480, 860)

        # Fluxo de agendamento: passo 1 (select barbeiro) → passo 2 (select serviço) → step 3
